...
import { statusLabel, fmtDate, progressPercent, progressLabel, extractItems, MEDIUMS, STATUSES, ORIGINS, onCoverError, roundToStep } from '../utils.jsx';
...
export default function Library({ initialFilters = {} }) {
  ...
  function TableColGroup({ columns, select = false, actions = false }) {
    return (
      <colgroup>
        {select && <col className="col-select" />}
        <col className="col-title" />
        {columns.map(col => (
          <col key={col} className={LIB_COLS[col]?.cls} />
        ))}
        {actions && <col className="action-cell" />}
      </colgroup>
    );
  }
  ...
  function renderManageCell(entry, col) {
    switch (col) {
      ...
      case 'medium':    return <td key={col} className="col-medium col-dim">{entry.medium || '—'}</td>;
      case 'year':      return <td key={col} className="col-year col-dim">{entry.year || '—'}</td>;
      ...
      case 'progress':  return <td key={col} className="col-progress col-dim">{progressLabel(entry)}</td>;
      case 'updated':   return <td key={col} className="col-updated col-dim">{fmtDate(entry.updated_at)}</td>;
      case 'completed': return <td key={col} className="col-completed col-dim">{fmtDate(entry.completed_at)}</td>;
  ...
  function renderViewCell(e, col) {
    switch (col) {
      case 'medium': return <td key={col} className="col-medium col-dim">{e.medium}</td>;
      case 'year':   return <td key={col} className="col-year col-dim">{e.year || '—'}</td>;
      case 'updated':   return <td key={col} className="col-updated col-dim">{fmtDate(e.updated_at)}</td>;
      case 'completed': return <td key={col} className="col-completed col-dim">{fmtDate(e.completed_at)}</td>;
      case 'progress': {
    ...
    <>
      ...
      <div className="batch-panel">
        ...
        <CustomSelect
          ...
          containerClassName="batch-select-control"
            ...
            <table className={`media-table library-table manage-entry-table${fixTitle ? ' media-table-fixed' : ''}`} data-mobile-show="status">
              ...
              <thead>
                <tr>
                  ...
                  <SortTh field="title" className="col-title">Title</SortTh>
