...
// ── CacheCoversModal ──────────────────────────────────────────────────────────
// Confirm + run the server-side cover cache job. The backend downloads every
// not-yet-cached cover and stores it; Cloudflare-gated covers (NovelUpdates)
// fail here and are surfaced as a count (those need the browser extension).

const EMPTY = { processed: 0, total: 0, cached: 0, skipped: 0, failed: 0 };

export default function CacheCoversModal({ onClose }) {
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ width: 520, display: 'flex', flexDirection: 'column' }}>
        <div className="modal-header">
          <span className="modal-title">Cache Covers Server-Side</span>
