...
/**
 * Inline (Console) duplicate finder: groups entries that share a (title, medium),
 * lets the user pick which one to keep per group and delete the rest — either one
 * group at a time, or every group at once via "Delete all". Entries can also be
 * edited in place (via EntryDetailModal, which remains a real overlay), since some
 * "duplicates" are really just mis-tagged entries that stop colliding once fixed.
 * Loads lazily when mounted, so the collapsible section only fetches when opened.
 */
export default function DedupPanel({ onResolved }) {
